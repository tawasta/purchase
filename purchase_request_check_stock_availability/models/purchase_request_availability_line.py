...
class PurchaseRequestAvailabilityLine(models.Model):
    ...
    request_line_id = fields.Many2one(
        comodel_name="purchase.request.line", string="Related Request Line",
    )
    ...
    analytic_account_id = fields.Many2one(
        related="request_line_id.analytic_account_id",
    ...
    def create_transfer(self):
        ...
        if self.product_qty <= transfer_qty:
            ...
            for line in self.request_line_id.availability_line_ids:
                line.active = False
            # Delete the actual purchase request line
            self.request_line_id.unlink()
        else:
