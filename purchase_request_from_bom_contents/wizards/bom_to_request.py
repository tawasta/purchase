...
class BomToRequest(models.TransientModel):
    ...
    _description = "Import BoM contents to purchase request"
    _rec_name = "bom_id"

    def calculate_quantities(self):
        # Fetch quantities of all materials using the
        # mrp_bom_raw_material_quantities module.
        material_quantities = self.bom_id.compute_raw_material_qties(
            multiplier=self.multiplier
        )

        for material in material_quantities:
            self.env["purchase_request_from_bom_contents.bom_to_request_line"].create(
                {
